...
def get_lightcurve(src_event_file,bkg_event_file,scale,user_defined_bti=None,
                      verbose=False, min_Frac_EXP=0.3,
                      CCDNR=4,input_timebin_size=50,PATTERN=4,PI=[200,10000],
    ...
    N_frames_per_bin = np.ceil(input_timebin_size*1000/FRAME_TIME).astype(int) # integer number of frames per bin
    timebin = N_frames_per_bin*FRAME_TIME/1000 # s
    if verbose: print(f'N_frames_per_bin = {N_frames_per_bin}')
    if verbose: print(f'timebin = {timebin} s')
    ...
    for iter,item in enumerate([src_event_file,bkg_event_file]):
        hdu = fits.open(item)
        if verbose: print(f'Number of events = {len(hdu["EVENTS"].data)}')
        ...
        if verbose: print(f'Number of events after filtering = {len(event_table)}')
        # get the time column
        ...
        count,t_bin= np.histogram(time,bins=bin_list,range=(t_start,t_end)) # counts and time at the start of each bin
        if verbose: print(f'Number of bins = {len(t_bin)-1}')
        ...
        if verbose: print(f'Number of GTIs = {len(GTI)}')
        if len(GTI)==1:
    ...
    if user_defined_bti is not None:
        ...
        plot_raw_lc(times[0],counts,[frac_exp,frac_exp],[user_defined_bti,user_defined_bti],filename='user_lc')

    # np.savetxt('frac_exp.txt',clean_Frac_EXP)

    # get the cleaned light curves and variance by rescaling the exposures
    cleaned_src = np.divide(counts[0].astype(float),clean_Frac_EXP,out=np.zeros_like(counts[0].astype(float)),where=clean_Frac_EXP!=0)
    cleaned_src2 = np.divide(counts[0].astype(float),clean_Frac_EXP**2,out=np.zeros_like(counts[0].astype(float)),where=clean_Frac_EXP!=0)

    cleaned_bkg = np.divide(counts[1].astype(float),clean_Frac_EXP,out=np.zeros_like(counts[1].astype(float)),where=clean_Frac_EXP!=0)
    cleaned_bkg2 = np.divide(counts[1].astype(float),clean_Frac_EXP**2,out=np.zeros_like(counts[1].astype(float)),where=clean_Frac_EXP!=0)
    
    if verbose: print(f'Min counts/bin in the src lc:',np.min(np.delete(cleaned_src,np.where(cleaned_src<=0))))
    ...
    if verbose: print(f'Number of bins = {len(t)}')
...
def calculate_Frac_EXP(t_bin,btis):
    ...
    for i in range(nbtis):
        ...
        if bti_end>=0:
            ...
            if size >0:
                Frac_EXP[bti_start] = Frac_EXP[bti_start] - (t_bin[bti_start+1] - btis[i][0])
